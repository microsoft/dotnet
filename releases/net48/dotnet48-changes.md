...
* Fixed font scaling issue when DPI changes. Winforms application and controls, when enabled for "per-Monitor" Dpi aware, are not scaled according to the DPI of the monitor (or device). Winform apps, by default, behaves like "System" DPI aware. This was causing Winforms applications/controls to be displayed blurry as a result of windows scaling them and - in some cases - controls are either not scaled or scaled out of proportion.
